...
def _get_vertex_express_url(
    ...
    if mode == "chat":
        ...
        url = f"{VERTEX_EXPRESS_BASE_URL}/v1/publishers/google/models/{model}:{endpoint}?key={api_key}"
        if stream:
            url += "&alt=sse"
    elif mode == "embedding":
        endpoint = "predict"
        ...
        raise ValueError(f"Unsupported mode for Vertex AI Express: {mode}")
